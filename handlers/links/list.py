...
class ListLinksHandler(webapp.RequestHandler):
    def get(self):
        ...
        for link in links:
            ...
            </tr>""" % (link.title, link.url, link.name, link.weight, link.group, link.modified_by.email(), link.modified_on, link.key(), link.name, link.group, link.group, link.key(), link.name)
