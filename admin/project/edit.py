...
class EditProjectHandler(webapp.RequestHandler):
  def get(self, url=None):
    ...
    <p>
      <form method="post" enctype="multipart/form-data">
        ...
        Current: <img src="http://second-bit.appspot.com/projects/%s/screenshot.png"><br />
        <label>Icon:</label>
        <input type="file" name="icon" /><br />
        Current: <img src="http://second-bit.appspot.com/projects/%s/icon.png"><br />
        <input type="submit" />
      </form>
  ...
  def post(self, url):
    ...
    if self.request.get('screenshot'):
      project_data.screenshot = self.request.get('screenshot')
    if self.request.get('icon'):
      project_data.icon = self.request.get('icon')
